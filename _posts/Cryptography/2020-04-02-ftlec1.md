...
$$\left[\begin{matrix} a&b\\c&d\end{matrix}\right]$$ with $a,b,c,d\in\mathbb{Z}_{26}$ is invertible iff its determinant $\Delta=ad-bc$ is invertible in $\mathbb{Z}_{26}$, and its inverse is $\Delta^{-1}\left[\begin{matrix}d&-b\\-c&a\end{matrix}\right]$
