...
$P_n(x)=f(x_0)+f[x_0,x_1](x-x_0)+\cdots+f[x_0,\cdots,x_n](x-x_0)\cdots(x-x_{n-1})$
...
$P_n(x)=f(x_n)+f[x_n,x_{n-1}](x-x_n)+\cdots+f[x_n,\cdots,x_0](x-x_n)\cdots(x-x_1)$
...
$h_0(x)=(1-2L'_{2,0}(x_0)(x-x_0))L^2_{2,0}(x)=(10x-12)(\frac{50}{9}x^2-\frac{175}{9}x+\frac{152}{9})^2$
...
因为$S_j(x)$是三次多项式，所以$S''_j(x)$是一次的，确定两个端点就可以得到该线性方程的表达式

假设$S''_j(x_{j-1})=M_{j-1},S''_j(x_j)=M_j$. (M被称为bending moment), 就像在则对于所有$x\in[x_{j-1},x_j]$
