...
$(t_i,f(t_i,y(t_i))),(t_{i-1},f(t_{i-1},y(t_{i-1}))),\cdots,(t_{i+1-m},f(t_{i+1-m},y(t_{i+1-m})))$

$$f(t,y(t))=P_{m-1}(t)+\frac{f^{(m)}(\xi_i,y(\xi_i))}{m}(t-t_i)(t-t_{i-1})\cdots(t-t_{i+1-m})$$



$$\begin{aligned} \int_{t_i}^{t_{i+1}}f(t,y(t))&=\int_{t_i}^{t_{i+1}}\sum_{k=0}^{m-1}(-1)^k \left[\begin{matrix}-s\\k\end{matrix}\right] \nabla^k f(t_i,y(t_i))dt \\&+\int_{t_i}^{t_{i+1}}\frac{f^{(m)}(\xi,y(\xi))}{m!}(t-t_i)(t-t_{i-1})\cdots (t-t_{i+1-m})dt \\&= \end{aligned}$$
